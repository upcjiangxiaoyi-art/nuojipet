...
这是 GPT × Ripple 一起搓、经 Fable51 工程审校的会眨眼版 `v0.3.2`。
...
`v0.2.0` 已将首版的圆滚滚程序造型替换为透明 2.5D 银白猫狐皮肤；`v0.3.0` 又加入了不换脸、不漂锚点的自然眨眼；`v0.3.1` 补齐透明点击穿透与省电清理；`v0.3.2` 修好移动端设置页的实时预览和主题按钮兼容。Canvas 继续负责呼吸、倾头、跳跃与状态特效。旧造型只在皮肤文件意外加载失败时应急出现。
