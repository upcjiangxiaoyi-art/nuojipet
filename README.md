...
这是 GPT × Ripple 一起搓、经 Fable51 工程审校的会眨眼、会甩尾、会明显动耳、会趴下、会缩成咕噜噜毛团，也会四只爪一起交替迈步来陪你的 `v0.8.2`。
...
- 巡游使用两组相反步态，前后爪会交替抬起、落地和后蹬，不再整只猫平移漂浮
...
`v0.2.0` 换上透明 2.5D 银白猫狐皮肤；`v0.3.x` 陆续加入眨眼、点击穿透、手机减负与可靠的流式生命周期；`v0.4.x` 让尾巴和两只耳朵按心情明显活动；`v0.5.0` 加入打字注视、久等陪伴话和三种触摸手势；`v0.6.0` 建起多形态框架并加入趴姿；`v0.7.0` 加入生成期间的滚动团子形态；`v0.8.0` 加入侧身转向与安全小步巡游。Canvas 继续负责呼吸、倾头、跳跃与状态特效。旧造型只在皮肤文件意外加载失败时应急出现。
...
- `assets/nuoji-walk-green-v1.png`：侧身抬爪的行走生产母版，左右方向由 Canvas 实时镜像
- `assets/nuoji-walk-green-v2.png`：相反落爪的第二步态母版，与第一帧交替形成真正迈步
...
- 进一步把侧身小步拆成腿部小骨骼，让步态从轻盈起伏升级为交替迈爪
