...
这是 GPT × Ripple 一起搓、经 Fable51 工程审校的会眨眼版 `v0.3.3`。
...
- 开心、被摸摸和睡觉时会用自身毛纹合成柔软的眯眼，不再像戴灰色墨镜
- 根据酒馆事件自动切换动作：
  - 发出或渲染用户消息：立刻思考
  - 命令处理完、即将生成：继续思考
  - 开始生成：思考
...
- 第三方输入流程漏发酒馆事件时，会用聊天记录新增作为打字指示兜底
- 不调用模型、不发送聊天内容、不额外消耗 token

`v0.2.0` 已将首版的圆滚滚程序造型替换为透明 2.5D 银白猫狐皮肤；`v0.3.0` 又加入了不换脸、不漂锚点的自然眨眼；`v0.3.1` 补齐透明点击穿透与省电清理；`v0.3.2` 修好移动端设置页的实时预览和主题按钮兼容；`v0.3.3` 则给 iPhone 设置预览减负、重做眯眼，并补齐输入后的思考指示。Canvas 继续负责呼吸、倾头、跳跃与状态特效。旧造型只在皮肤文件意外加载失败时应急出现。
