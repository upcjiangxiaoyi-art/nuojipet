...
这是 GPT × Ripple 一起搓、经 Fable51 工程审校的会眨眼版 `v0.3.5`。
...
- 手机按下酒馆原生发送键时直接进入思考；停止生成键出现与稳定消失会确认整个流式周期
- 流式回复刚创建消息壳或过早发出结束事件时不会提前回到待机
- 第三方输入流程漏发酒馆事件时，还会用聊天记录新增作为第二层兜底
- 设置面板显示最近一次联动状态，手机上无需控制台也能判断有没有收到发送信号
- 不调用模型、不发送聊天内容、不额外消耗 token

`v0.2.0` 已将首版的圆滚滚程序造型替换为透明 2.5D 银白猫狐皮肤；`v0.3.0` 加入自然眨眼；`v0.3.1` 补齐透明点击穿透与省电清理；`v0.3.2` 修好移动端设置页预览和主题按钮兼容；`v0.3.3` 给 iPhone 设置预览减负；`v0.3.4` 改用真正的闭眼眼周素材，并把打字指示器接到发送键和停止键本体；`v0.3.5` 修正流式回复过早结束的判断，让思考气泡常驻并加入点头。Canvas 继续负责呼吸、倾头、跳跃与状态特效。旧造型只在皮肤文件意外加载失败时应急出现。
